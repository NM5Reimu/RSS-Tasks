class Calculator {
    ...
    computeUnaryOperation(operation){
        ...
        switch (operation) {
            case '√':
                if(current < 0) {
                    alert('Ошиб_очка. Не стоит вскрывать эту тему.') ;
                    computation = '';
                }
  ...
  const unaryOperationButtons = document.querySelectorAll('[data-operation-u]');
  ...
  unaryOperationButtons.forEach(button => {
        button.addEventListener('click', () => {
            calculator.computeUnaryOperation(button.innerText);
            calculator.updateDisplay();
        })
  })